...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateBankAccountRequest$outboundSchema.optional().parse(
        value,
      ),
  ...
  const path = pathToFunc("/bank-accounts")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, operations.CreateBankAccountResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
