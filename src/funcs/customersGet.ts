...
export function customersGet(
  ...
  Result<
    operations.GetCustomerByIdResponse,
    | MiddayError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetCustomerByIdRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetCustomerByIdResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json("default", operations.GetCustomerByIdResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
