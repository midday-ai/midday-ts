...
export function inboxGet(
  ...
  Result<
    operations.GetInboxItemByIdResponse,
    | MiddayError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetInboxItemByIdRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetInboxItemByIdResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json("default", operations.GetInboxItemByIdResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
