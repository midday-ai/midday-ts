...
export function invoicesGet(
  ...
  Result<
    operations.GetInvoiceByIdResponse,
    | MiddayError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetInvoiceByIdRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetInvoiceByIdResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json("default", operations.GetInvoiceByIdResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
