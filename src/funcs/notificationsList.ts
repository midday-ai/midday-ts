...
export function notificationsList(
  ...
  Result<
    operations.ListNotificationsResponse,
    | MiddayError
...
> {
  ...
  const query = encodeFormQuery({
    ...
    "maxPriority": payload.maxPriority,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListNotificationsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json("default", operations.ListNotificationsResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
