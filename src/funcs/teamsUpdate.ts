...
export function teamsUpdate(
  ...
  Result<
    operations.UpdateTeamByIdResponse,
    | MiddayError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateTeamByIdRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/teams/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.UpdateTeamByIdResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json("default", operations.UpdateTeamByIdResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
