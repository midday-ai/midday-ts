...
export function transactionsDelete(
  ...
  Result<
    operations.DeleteTransactionResponse,
    | MiddayError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.DeleteTransactionRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.DeleteTransactionResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
    M.json("default", operations.DeleteTransactionResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
